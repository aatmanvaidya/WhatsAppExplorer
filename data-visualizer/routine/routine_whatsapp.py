...
def detect_nude(media_directory, thumbnail_path, image_files, nude_probability_map={}):
    thumbnail_files = set()
    ...
    del existing_nude_files
    del combined_image_files
    ...
    try:
        with open(csv_file_path, 'a', newline='') as csvfile:
            ...
            for filename in remaining_nude_files:
                file_path = os.path.join(media_directory, filename)

                if filename in thumbnail_files:
...
def whatsapp():
    ...
    def extract_thumbnails(media_directory, thumbnail_path, video_files, video_files_no_ext):
        if not os.path.exists(thumbnail_path):
            os.mkdir(thumbnail_path)
        
        # Calculating delta for thumbnails
        ...
        for filename in remaining_thumbnail_files:
            
            video_path = os.path.join(media_directory, filename)
    ...
    extract_thumbnails(media_directory, thumbnail_path, video_files, video_files_no_ext)
    ...
    if detect_nude(media_directory, thumbnail_path, image_files, nude_probability_map) != 0:
    ...
    del hashed_video_files_no_ext
    del video_files_no_ext
    del unhashed_video_files_no_ext
    ...
    with open(haystack_file_path, 'a') as haystack_file:
        for file in unhashed_image_files:
            file_path = os.path.join(media_directory, file)
            command = f"./pdq_old/bin/pdq-photo-hasher-tool --details '{file_path}'"
    ...
    response = run_command("./pdq/clusterize256x {}/haystack.hsh > {}/haystack.clu".format(temp_directory, temp_directory))
    if response != 0:
    ...
    file_clusters = {}
    with open("{}/haystack.clu".format(temp_directory), 'r') as file:
        for line in file:
