...
def compute_video_hash_and_cluster(temp_directory, media_directory, corrupt_videos):
...
def compute_image_clusters(temp_directory, media_directory):
    ...
    return (image_map, corrupt_images)
...
def whatsapp():
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        with Manager() as manager:
            ...
            yesterday_date = (datetime.datetime.utcnow() - datetime.timedelta(days=4)).replace(hour=0, minute=0, second=0, microsecond=0)
            ...
            nude_probability_map = manager.dict()
            future_detect_nude = executor.submit(detect_nude, media_directory, nude_probability_map)
            future_compute_video_hash_and_cluster = executor.submit(compute_video_hash_and_cluster, temp_directory, media_directory, corrupt_videos)
            future_compute_image_clusters = executor.submit(compute_image_clusters, temp_directory, media_directory)
            
            concurrent.futures.wait([future_compute_video_hash_and_cluster, future_detect_nude, future_compute_image_clusters])

            result_detect_nude = future_detect_nude.result()
            result_compute_video_hash_and_cluster = future_compute_video_hash_and_cluster.result()
            result_compute_image_clusters = future_compute_image_clusters.result()
            
            if result_detect_nude != 0:
                print("Crashed while running nude detection. Terminating!")
                return False
            
            print("Nude detection completed.")
            
            if result_compute_video_hash_and_cluster == False:
                print("Crashed while running video clustering. Terminating!")
                return False
            video_map = result_compute_video_hash_and_cluster
            
            print("Video clustering completed.")
            
            if result_compute_image_clusters == False:
                print("Crashed while running image clustering. Terminating!")
                return False
            image_map, corrupt_images = result_compute_image_clusters
            ...
            with open("{}/text_msg_uuid_wa1.json".format(temp_directory), "w", encoding='utf8') as outfile:
                json.dump([{'message': message, 'uuid': uid} for message, uid in text_message_uuid_map.items()], outfile, ensure_ascii=False)
            with open("{}/image_wa1.json".format(temp_directory), "w", encoding='utf8') as outfile:
                json.dump(data_list["image"], outfile, ensure_ascii=False)
            with open("{}/video_wa1.json".format(temp_directory), "w", encoding='utf8') as outfile:
                json.dump(data_list["video"], outfile, ensure_ascii=False)
            with open("{}/text_msg_wa1.json".format(temp_directory), "w", encoding='utf8') as outfile:
                json.dump(data_list["text_msg"], outfile, ensure_ascii=False)
            with open("{}/link_wa1.json".format(temp_directory), "w", encoding='utf8') as outfile:
...
if status == True:
    ...
    if run_command('mongo {} --eval "db.messagesUuidMap.deleteMany({{}})"'.format(database)) == 0:
        run_command("mongoimport --jsonArray --db {} --collection messagesUuidMap --file {}/text_msg_uuid_wa1.json --legacy".format(database, temp_directory_wa))
    
    if run_command('mongo {} --eval "db.links.deleteMany({})"'.format(database, platform_str)) == 0:
        run_command("mongoimport --jsonArray --db {} --collection links --file {}/link_wa1.json --legacy".format(database, temp_directory_wa))
    
    if run_command('mongo {} --eval "db.messages.deleteMany({})"'.format(database, platform_str)) == 0:
        run_command("mongoimport --jsonArray --db {} --collection messages --file {}/text_msg_wa1.json --legacy".format(database, temp_directory_wa))
    
    if run_command('mongo {} --eval "db.images.deleteMany({})"'.format(database, platform_str)) == 0:
        run_command("mongoimport --jsonArray --db {} --collection images --file {}/image_wa1.json --legacy".format(database, temp_directory_wa))
    
    if run_command('mongo {} --eval "db.videos.deleteMany({})"'.format(database, platform_str)) == 0:
        run_command("mongoimport --jsonArray --db {} --collection videos --file {}/video_wa1.json --legacy".format(database, temp_directory_wa))
