    ...
    for key in data:
        try:
            ...
            if key == "text_msg":
                ...
                for section in data[key]:
                    for message in data[key][section]:
                        data_list[key].append({"content": message, "senderData":  remove_redundant_copies(data[key][section][message]["senderData"]), "platform": "whatsapp", "section": section, "flags": [], "isDownloaded": data[key][section][message]["isDownloaded"], "isExplicit": data[key][section][message]["isExplicit"]})
            elif key == "image":
                for section in data[key]:
                    for message in data[key][section]:
