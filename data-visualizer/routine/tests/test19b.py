...
def test():
    ...
    cluster_file = "../vidstackk.clu"

    # Get a list of .tmk files in the source directory
    tmk_files = [file for file in os.listdir("./") if file.endswith(".tmk")]
    ...
    for tmk_file in tmk_files:
        tmk_path = os.path.join("./", tmk_file)
    ...
    for mp4_size, tmk_paths in files_by_size.items():
        ...
        if len((batch_tmk_paths + " " + tmk_paths_str).strip()) > threshold:
            # run clustering
